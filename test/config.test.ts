...
const managed = ["MEDIA_ROOTS", "CACHE_DIR", "CONFIG_DIR", "WORKERS", "DRY_RUN", "SCAN_INTERVAL_MINUTES", "MIN_SAVINGS_PERCENT", "QSV_QUALITY", "QSV_PRESET", "QSV_DEVICE", "SONARR_URL", "SONARR_API_KEY", "RADARR_URL", "RADARR_API_KEY"];
...
test("safe defaults include dry-run and two workers", () => {
  ...
  try {
    ...
    assert.deepEqual(config.roots, ["/data/TV", "/data/Movies"]);
